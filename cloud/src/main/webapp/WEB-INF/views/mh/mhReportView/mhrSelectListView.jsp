...
<!-- content -->
			...
			<div class="row text-start offset-md-3 my-4">
				<form class="col row" action="mhrSelectList" name="searchForm" id="searchForm" method="post">
						 <select name="mhrType" class="border border-end-0" style="left:0px;width:102px; height:54px;">
		                     <option value="title" ${mhrType == 'title'? 'selected' : '' }>제 목</option>
		                     <option value="writer" ${mhrType == 'writer'? 'selected' : '' }>작성자</option>
		                     <option value="content" ${mhrType == 'content'? 'selected' : '' }>내용</option>
	                   </select>

					<div class="col text-start p-0">
						<input type="text" name="mhrKeyword" value="${mhrKeyword}" style="left:0px; width:500px; height:54px; background:#fff; border:1px solid #d0d0d0;"><input type="submit" class="bg-danger text-white bg-opacity-75" style="left:0px; top:0px; width:54px; height:56px; background:#fff;  border:1px solid #d0d0d0;" value="검색">
